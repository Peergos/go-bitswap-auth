...
type timeoutRecorder struct {
	timedOutKs []auth.Want
	lk         sync.Mutex
}

func (tr *timeoutRecorder) onTimeout(tks []auth.Want) {
	tr.lk.Lock()
	defer tr.lk.Unlock()
...
func TestDontHaveTimeoutMgrTimeout(t *testing.T) {
	firstks := testutil.GenerateWants(2)
	secondks := append(firstks, testutil.GenerateWants(3)...)
